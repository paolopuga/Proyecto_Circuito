...
Resultados::Resultados(const string & nombre)
{
    LeerResultados(nombre); // Leemos los datos
}
...
bool Resultados::EstaVacio(void) const
{
...
Resultados operator+(const Resultado& uno, const Resultados otro)
{
    ...
    resultado.Aniade(uno); // Añadimos el objeto

    return resultado + otro; // Concatenamos los objetos
}
...
istream& operator>>(istream& is, Resultados& objeto)
{
    objeto.LiberarMemoria(); // Liberamos la memoria

    Resultado resultado; // Creamos un objeto Resultado

    while (is >> resultado) // Mientras se pueda leer
    {
        objeto+=resultado; // Añadimos el objeto
    }
...
ostream& operator<<(ostream& os, const Resultados& objeto)
{
...
void Resultados::LeerResultados(const string & nombre)
{
    ...
    if (fichero && cad_mag == "RESULTADOS") // Si se ha abierto
    {
